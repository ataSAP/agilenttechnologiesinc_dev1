...
class CL_SalesforceIntegrationModules(CL_CpqHelper):
    ...
    ###############################################################################################
    def call_rest_api(self, url, headers, body, method, integrationReference):
        response = None
    ...
    def get_admin_auth2_token(self):
        accessToken = str()
        if not self.Session["adminApiSessionID"]:
            url = CL_SalesforceSettings.SALESFORCE_URL + API.AUTH_API
            # Get Admin access_token
            response = AuthorizedRestClient.GetPasswordGrantOAuthToken(CL_SalesforceSettings.SALESFORCE_PWD, CL_SalesforceSettings.SALESFORCE_SECRET, url)
            if response["access_token"] != "" and response["access_token"] is not None:
                self.Session["adminApiSessionID"] = accessToken = str(response.access_token)
            if accessToken == "":
                Log.Error("CPQ-SFDC Integration: Authentication Error", str(response))
        else:
            accessToken = self.Session["adminApiSessionID"]
        return accessToken
    ...
    ###############################################################################################
    def post_composite_request(self, bearerToken, compositePayload, integrationReference):
        # API path
        ...
        response = self.call_rest_api(url, headers, body, API.POST, integrationReference)
        return response

    ###############################################################################################
    # Function to call POST SObject Collection API
    ###############################################################################################
    def post_sobjectcollection_request(self, headers, body, integrationReference):
        # API path
    ...
    ###############################################################################################
    def call_soql_api(self, headers, soql, integrationReference=None):
        # API path
        ...
        response = self.call_rest_api(url, headers, None, API.GET, integrationReference)
        return response

    ###############################################################################################
    # Function to call GET SOQL API
    ###############################################################################################
    def call_sobject_delete_api(self, bearerToken, url, integrationReference):
        headers = self.get_authorization_header(bearerToken)
        response = self.call_rest_api(url, headers, None, API.DELETE, integrationReference)
        return response
    ...
    def build_cr_get_opp_quotes(self, opportunityId):
        url = API.CR_GET_OPP_QUOTES_API.format(opportunityId=str(opportunityId))
        compositeRequest = self.build_cr_sobject_request(url, API.GET, None, REF.GET_OPP_QUOTES_REFID)
        return compositeRequest
    ###############################################################################################
    ...
    def build_cr_record_create_opportunity(self):
        ...
        if oppStatus:
            ...
            self.Quote.Messages.Add(CL_IntegrationMessages.NO_STATUS_MAPPING)
    ...
    def build_cr_record_update_opportunity(self, opportunityId):
        ...
        if oppStatus:
            ...
            self.Quote.AddMessage(CL_IntegrationMessages.NO_STATUS_MAPPING, MessageLevel.Warning, False)
    ...
    def set_business_partner_id(self, cpqPartnerFunction, salesforceId, businessPartner=None):
        ...
        if businessPartner:
            ...
            if crmIdMapping:
                ...
                if crmIdMapping["CpqFieldType"] == CPQ_BP_STANDARD_FIELD:
                    # Set Id in CPQ
                    setattr(businessPartner, crmIdMapping["CpqField"], salesforceId)
                elif crmIdMapping["CpqFieldType"] == CPQ_BP_CUSTOM_FIELD:
                    # Set Id in BP custom field
                    businessPartner.Item[crmIdMapping["CpqField"]] = salesforceId
